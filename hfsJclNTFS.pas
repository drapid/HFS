...
uses
  {$IFDEF HAS_UNITSCOPE}
  System.Win.ComObj, System.Win.ShlObj,
  {$ELSE ~HAS_UNITSCOPE}
  ComObj,
  {$ENDIF ~HAS_UNITSCOPE}
...
begin
  ...
  if Result then
  begin
    if not hfsJclOthers.DirectoryExists(MountPoint) then
      hfsJclOthers.ForceDirectories(MountPoint);
